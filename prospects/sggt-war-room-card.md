---
title: SGGT War Room Card — Single Page, Print Large, Tape Next to Camera
type: opportunity
status: pitch-prep
event_datetime: 2026-05-15 13:30-17:30 CET
related: [_personal/prospects/sggt-live-pitch-2026-05-15, _personal/prospects/sggt-jury-followup-response-2026-tight]
updated: 2026-05-15
---
...
## Q&A TRIGGERS (10 min total, glance only)

```
 1. REFS          — Hakan + AFG, contacts in chat now (after consent check)
 2. DOC COUNT     — [PRE-WRITE actual number from AWS] ± send exact in chat
 3. FP/FN DEMO    — FP: 1.4×10⁴ vs 14000 KG (notation normaliser)
                    FN: phyto "free from Tilletia indica" missing on
                    India-bound wheat, human had passed
 4. MCP CUSTOMER  — Hakan enabled. We DON'T instrument cross-tenant traffic
                    BY DESIGN — that's what isolation means.
 5. ARR           — < $2M combined. Growth = TrackonAI + TradingDocs.
 6. EVIDENCE      — same input (6 docs), same output (discrepancy list),
                    variable = human highlighter vs AI list.
                    OFFER live stopwatch on jury sample.
 7. RULESET REFRESH— rare. Once stable, treated as stable.
                    Every shipment cites versioned ruleset → audit benefit.
                    Continuous push on roadmap.
 8. DEFENSIBILITY — (a) domain DNA (20 yrs grain)
                    (b) integrated stack (4 categories spanned)
                    (c) per-tenant learning loop (6 mo head start)
 9. WHY GRAIN     — wedge not limit. Sugar/pulses/fertiliser/coffee already
                    running. Dairy advanced. Proving range is why we're here.
10. ASK           — IMEAA network > jury intros > prize cheque.
11. HEDGING       — Module BUILT (Hedge Book / Instruments / Exposure /
                    Effectiveness / P&L / Scenarios) BUT adoption light —
                    current customers are physical traders who hedge with
                    brokers. Ready for upmarket / DFA conversation.
                    NO live exchange feed today.
...
## CRITICAL FACTS (numbers/names — must be right)

```
- Hakan Foods DMCC       — paying, grain, TrackonAI, consented [VERIFY today]
- AFG Trading DMCC       — paying, grain, TrackonAI, consented [VERIFY today]
- DFA (Dairy Farmers     — world's #3 dairy, advanced commercial discussion
  of America)              (NOT a customer yet; never say customer)
- UAE trading house      — active trial (NDA, don't name)
