...
- [ ] Demo tenant: one clean trade thread staged (purchase, sale, journey with steps and 2 expenses, one invoice, one payment request), plus one open position for the MTM moment. **Commodity: fertilizer.** Fatih's background is urea, NPK, sulphur, phosphoric acid, ammonia, base oils (see [[_personal/prospects/yalcintas-akdogan-newco]]). Suggested example: **granular urea, 10,000 t, purchased FOB Gulf producer (payment: advance or LC), sold CFR Mersin or CFR West Coast India under LC.** Bulk vessel, one shipment. Costs to stage: ocean freight (the big one), inspection/draft survey, bagging if bagged, demurrage/despatch line, LC bank charges. Second thread if time: base oils in flexitanks/drums per container, to show per-container invoicing. Leave 3,000 t of the purchase unsold so the position report shows exposure and MTM moves with an Argus/CRU style market price.
