...
Follow-through:
1. Switch plan in Admin Console → Billing → Subscriptions → Business Standard → change Flexible to Annual plan. **2026-07-29: actual checkout price confirmed — €163.20/user/yr (€13.60/user/mo) = €1,142.40/yr for 7 users, saving €218.40/yr (16%). Contract start 29 Jul 2026, renewal Jul 2027. Advised: prefer "Annual plan (monthly payment)" variant if offered (same price, no prepayment); yearly-payment variant acceptable otherwise.** After checkout: verify renewal settings in Billing.
